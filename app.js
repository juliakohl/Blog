...
//db.run('CREATE TABLE posts(title text, body text)');
...
app.get("/", function (req, res) {
  let db = new sqlite3.Database('db/blog.db');
...
app.get("/posts/:title", function(req,res){
  ...
  if(match){
    res.render("post", {title: output.title, text: output.body});
  } else {
...
app.post("/compose", function (req,res) {
  ...
  db.run('INSERT INTO posts VALUES(?, ?)', [title, body],function(err) {
    if (err) {
